...
def reg_existing_platform():
    ...
    if(Credentials.account_exist(ex_platform, ex_username)):
        ...
        new_account = (ex_platform, ex_username, ex_password)
        # Adding new account if it doesnt exist
        Credentials.add_account(new_account)
        print(f'Account {ex_username} added to platform {ex_platform}')
...
def reg_new_platform():
    ...
    if(nw_password == 'auto' or nw_password == 'AUTO'):
        password = password(int(input('Enter preferred length: ')))
    ...
    if input('Type CP to copy password').lower() == 'cp':
        ...
        if input('Type LS to list all your credentials or press any key to return to main').lower() == 'ls':
            Credentials.list_all_credentials()
        else:
            login_account()
    reg_new_platform()
